...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE send_buf[2];
	send_buf[0] = reg + 2; //send reg
	send_buf[1] = val; //send data
	int status = XSpi_Transfer(&SpiInstance, send_buf, NULL, 2);
	if (status != 0){
		xil_printf ("Error. Could not write to register.");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int status = XSpi_Transfer(&SpiInstance, send_buf, NULL, nbytes + 1);
	if (status != 0){
			xil_printf ("Error. Could not write to register.");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	...
	//XSpi_SetSlaveSelect(InstancePtr, Mask)
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	receive_buf[0] = 0;
	receive_buf[1] = 0;
	int status = XSpi_Transfer(&SpiInstance, send_buf, receive_buf, 2);
	if (status != 0){
		xil_printf ("Error. Could not read from register.");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int status = XSpi_Transfer(&SpiInstance, send_buf, receive_buf, nbytes + 1);
	if (status != 0){
		xil_printf ("Error. Could not read from register.");
	}
	for (int i = 0; i < nbytes; i++){
